...
Form templates are used to customize the user interface by allowing to choose field visibility and default values. The user can then use them for data entry on a single page or through a split operation.
...
The Split configuration includes a condition used to decide when to display the specific template for a split operation. The Name Generation section refers to 
the strategy to use for naming material-sample in a <<split>> operation.
