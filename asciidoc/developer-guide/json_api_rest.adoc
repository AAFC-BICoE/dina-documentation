...
Get the persons where either the firstName or the lastName is John:
----
GET /person?(filter[firstName][EQ]=John|filter[lastName][EQ]=John)
----
...
Logical filter expressions can be grouped using parentheses and support both OR (`|`) and AND (`&`) operators:
----
GET /person?((filter[firstName][EQ]=John|filter[lastName][EQ]=John)&filter[group][EQ]=admin)
----

NOTE: Multiple top-level filter parameters remain implicitly combined using AND. The `|` operator is primarily useful when grouping filter expressions.
