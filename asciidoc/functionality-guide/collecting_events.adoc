[id=collectingEvents]
== Collecting Events
Darwin Core defines "event" as : An action that occurs at some location during some time. (from https://dwc.tdwg.org/terms/#event[DarwinCore - Event]). In a Collecting Event, the action is the extraction from nature. (from https://aafc-bicoe.github.io/dina-documentation/#collecting-event[AAFC-BICoE DINA User Guide - Collecting Event])
