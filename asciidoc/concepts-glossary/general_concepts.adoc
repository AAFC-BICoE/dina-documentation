...
[[not_publicly_releasable]]
== Not Publicly Releasable

Not Publicly Releasable is a flag that can be set on certain resources to indicate that the data should not be made publicly available. 
Once a resource in the object store is marked as Not Publicly Releasable, users who do not have access to the resource's group will not be able to download it.
When a resource is marked as Not Publicly Releasable a reason can be provided to explain why the resource is restricted. 
This field can also be used to ensure data is not inadvertently shared when exporting data from DINA with automated tasks.

image::not_publicly_releasable.png[Not Publicly Releasable and Not Publicly Releasable Reason field on a resource form]

image::not_publicly_releasable_off.png[Not Publicly Releasable toggled off with no reason field visible]
